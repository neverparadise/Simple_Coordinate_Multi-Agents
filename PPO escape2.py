...
# 5. 네트워크 두 개 정의
mover_network = PPO(mover_obs_size, mover_action_size)
jumper_network = PPO(jumper_obs_size, jumper_action_size)
...
for n_epi in range(10000):
    ...
    while not done:
        for t in range(10):
            ...
            # action sampling from norm
            m_mu, m_std = mover_network.pi(torch.from_numpy(m_obs).float())
            m_dist = Normal(m_mu, m_std)
            m_a = m_dist.sample()
            m_log_prob = m_dist.log_prob(m_a)

            j_mu, j_std = jumper_network.pi(torch.from_numpy(j_obs).float())
            j_dist = Normal(j_mu, j_std)
            j_a = j_dist.sample()
            j_log_prob = j_dist.log_prob(j_a)
            ...
            mover_network.put_data((m_obs, m_log_prob, m_decision_steps.reward, m_nobs, done))
            jumper_network.put_data((j_obs, j_log_prob, j_decision_steps.reward, j_nobs, done))
